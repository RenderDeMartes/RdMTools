...
if cmds.window ("ChickenHead", exists = True):
    cmds.deleteUI ("ChickenHead")
...
############################    
    ...
    cmds.circle (n = 'Head_CC',r=ControlRadius*3*GlobalMult, nr = (1,0,0))
...
cmds.window( "ChickenHead", width=250, title="Head", toolbox = True)
cmds.columnLayout( adjustableColumn=True )
cmds.separator(h = 10)
cmds.symbolButton(image=Icons + 'Locator.png', c = LocatorsChickenHead)
cmds.separator(h = 10)
cmds.symbolButton(image=Icons + 'Joint.png', c = JointsChickenHead)
cmds.separator(h = 10)
RadioControlador = cmds.floatSliderGrp (l = 'Radio:', min = 1 , max = 20, field = True, v = 1 , hlc =( 1.000, 0.8, 0.0))
cmds.separator(h = 10)
cmds.button( label='Create Head', command=ChickenHead,  bgc =( 1.000, 0.8, 0.0), h = 40 )
cmds.separator(h = 10)
cmds.rowLayout(nc= 3, 
               columnAlign=(1, 'right'), 
               columnAttach=[(1, 'both', 50), 
                             (2, 'both', 50), 
                             (3, 'both', 0)] )
                                        
cmds.checkBox (l= 'Jaw',hlc =( 1.000, 0.8, 0.0),v = 1 )
cmds.checkBox (l= 'Eyes',hlc =( 1.000, 0.8, 0.0), v = 1)
cmds.button( label='Create Extras', command=Extras,  bgc =( 1.000, 0.8, 0.0), h = 40 )
cmds.setParent('..')
cmds.separator(h = 10)
cmds.text( label='By: Render de Martes', fn ='fixedWidthFont')
cmds.text( label='info@renderdemartes.com', fn ='fixedWidthFont')
cmds.showWindow("ChickenHead")
